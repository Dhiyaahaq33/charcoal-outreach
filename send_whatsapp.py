...
def has_office_format(*raw_fields):
    """True kalau salah satu field nomor (Phone/WhatsApp, ditulis APA ADANYA dari sheet, bukan
    hasil extract_number) pakai tanda kurung, misal "(021) 555-1234" - per user, itu ciri khas
    format nomor kantor/kode area, BUKAN nomor mobile. Nomor tanpa tanda kurung dianggap aman/
    WA-capable. Revisi dari deteksi berbasis library phonenumbers sebelumnya (dianggap kurang
    akurat) ke rule sederhana ini sesuai observasi langsung di data sheet."""
    return any(("(" in str(f) or ")" in str(f)) for f in raw_fields if f)
