...
Run manual: python scripts/check_quota.py
"""
...
today_wib = datetime.now(ZoneInfo("Asia/Jakarta")).strftime("%d/%m/%Y")
email_today = get_daily_email_count(core_ws, today_wib)
print(f"WA quota saat ini: {quota}")
print(f"Email terkirim hari ini ({today_wib} WIB): {email_today}")
